    ...
    public class CreacionUsuarioController : ControllerBase
    {
        ...
        public dynamic Login([FromBody] Usuario usurio)
        {
            ...
            try
            {
                ...
                var encrictkey = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

                var token = new JwtSecurityToken(jwt.Issuer, jwt.Audience, claims, expires: DateTime.Now.AddHours(1), signingCredentials: encrictkey);
        ...
        public ActionResult<UsuarioDto> crearUsuario([FromBody] UsuarioDto usuario, string JwtToken, string email)

        {
            ...
            try
            {
                ...
                if (!validarToken(JwtToken, email))
                {
                    ModelState.AddModelError("mensaje", "token o correo invalido.");
